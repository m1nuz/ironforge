...
namespace video {
    ...
    namespace vertgen {
        ...
        auto make_plane(const glm::mat4 &transform) -> vertices_info {
            ...
            sh.vertices = {vec4{-1.0f, -1.0f, 0.0f, +1.0f},
                           vec4{+1.0f, -1.0f, 0.0f, +1.0f},
                           vec4{-1.0f, +1.0f, 0.0f, +1.0f},
                           vec4{+1.0f, +1.0f, 0.0f, +1.0f}};
            sh.normals = {vec3{0.0f, 0.0f, 1.0f},
                          vec3{0.0f, 0.0f, 1.0f},
                          vec3{0.0f, 0.0f, 1.0f},
                          vec3{0.0f, 0.0f, 1.0f}};
            sh.tangents = {vec3{1.0f, 0.0f, 0.0f},
                           vec3{1.0f, 0.0f, 0.0f},
                           vec3{1.0f, 0.0f, 0.0f},
                           vec3{1.0f, 0.0f, 0.0f}};
            sh.texcoords = {vec2{0.0f, 0.0f},
                            vec2{1.0f, 0.0f},
                            vec2{0.0f, 1.0f},
                            vec2{1.0f, 1.0f}};
        ...
        auto make_cube(const glm::mat4 &transform) -> vertices_info {
            ...
            sh.attributes = nullptr;
            sh.mode = GL_TRIANGLES;
            sh.vertices = {vec4{-1.0f, -1.0f, -1.0f, +1.0f}, vec4{-1.0f, -1.0f, +1.0f, +1.0f}, vec4{+1.0f, -1.0f, +1.0f, +1.0f}, vec4{+1.0f, -1.0f, -1.0f, +1.0f},
                           vec4{-1.0f, +1.0f, -1.0f, +1.0f}, vec4{-1.0f, +1.0f, +1.0f, +1.0f}, vec4{+1.0f, +1.0f, +1.0f, +1.0f}, vec4{+1.0f, +1.0f, -1.0f, +1.0f},
                           vec4{-1.0f, -1.0f, -1.0f, +1.0f}, vec4{-1.0f, +1.0f, -1.0f, +1.0f}, vec4{+1.0f, +1.0f, -1.0f, +1.0f}, vec4{+1.0f, -1.0f, -1.0f, +1.0f},
                           vec4{-1.0f, -1.0f, +1.0f, +1.0f}, vec4{-1.0f, +1.0f, +1.0f, +1.0f}, vec4{+1.0f, +1.0f, +1.0f, +1.0f}, vec4{+1.0f, -1.0f, +1.0f, +1.0f},
                           vec4{-1.0f, -1.0f, -1.0f, +1.0f}, vec4{-1.0f, -1.0f, +1.0f, +1.0f}, vec4{-1.0f, +1.0f, +1.0f, +1.0f}, vec4{-1.0f, +1.0f, -1.0f, +1.0f},
                           vec4{+1.0f, -1.0f, -1.0f, +1.0f}, vec4{+1.0f, -1.0f, +1.0f, +1.0f}, vec4{+1.0f, +1.0f, +1.0f, +1.0f}, vec4{+1.0f, +1.0f, -1.0f, +1.0f}};
            sh.normals = {vec3{ 0.0f, -1.0f,  0.0f}, vec3{ 0.0f, -1.0f,  0.0f}, vec3{ 0.0f, -1.0f,  0.0f}, vec3{ 0.0f, -1.0f,  0.0f},
                          ...
                          vec3{+1.0f,  0.0f,  0.0f}, vec3{+1.0f,  0.0f,  0.0f}, vec3{+1.0f,  0.0f,  0.0f}, vec3{+1.0f,  0.0f,  0.0f}};
            sh.tangents = {vec3{+1.0f,  0.0f,  0.0f}, vec3{+1.0f,  0.0f,  0.0f}, vec3{+1.0f,  0.0f,  0.0f}, vec3{+1.0f,  0.0f,  0.0f},
                           vec3{+1.0f,  0.0f,  0.0f}, vec3{+1.0f,  0.0f,  0.0f}, vec3{+1.0f,  0.0f,  0.0f}, vec3{+1.0f,  0.0f,  0.0f},
                           vec3{-1.0f,  0.0f,  0.0f}, vec3{-1.0f,  0.0f,  0.0f}, vec3{-1.0f,  0.0f,  0.0f}, vec3{-1.0f,  0.0f,  0.0f},
                           vec3{+1.0f,  0.0f,  0.0f}, vec3{+1.0f,  0.0f,  0.0f}, vec3{+1.0f,  0.0f,  0.0f}, vec3{+1.0f,  0.0f,  0.0f},
                           vec3{ 0.0f,  0.0f, +1.0f}, vec3{ 0.0f,  0.0f, +1.0f}, vec3{ 0.0f,  0.0f, +1.0f}, vec3{ 0.0f,  0.0f, +1.0f},
                           vec3{ 0.0f,  0.0f, -1.0f}, vec3{ 0.0f,  0.0f, -1.0f}, vec3{ 0.0f,  0.0f, -1.0f}, vec3{ 0.0f,  0.0f, -1.0f}};
            sh.texcoords = {vec2{0.0f, 0.0f}, vec2{0.0f, 1.0f}, vec2{1.0f, 1.0f}, vec2{1.0f, 0.0f},
        ...
        auto make_sphere(const gen_sphere_info *info, const glm::mat4 &transform) -> vertices_info {
            ...
            for (size_t i = 0; i < num_parallels + 1; i++)
                for (size_t j = 0; j < num_slices + 1; j++) {
                    ...
                    glm::quat h{static_cast<float>(sh.texcoords.back().x * 2.f * M_PI), 0.f, 0.f, 0.f};
